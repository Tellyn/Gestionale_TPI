...
class EnterExitRepository
{
    ...
    public static function get_last_uscita()
    {
        $user = Authenticator::getUser();
        $pdo = Connection::getInstance();
        $sql = 'SELECT Uscita.ID ,orario_uscita,id_entrata FROM Uscita INNER JOIN Giorno ON id_giorno=Giorno.ID WHERE Giorno.id_utente=:utente ORDER BY Uscita.ID DESC LIMIT 1 ';
        $stmt = $pdo->prepare($sql);
        $stmt->execute(
            [
                'utente' => $user['ID']
    ...
    public static function get_uscite_entrate(){
        $user = Authenticator::getUser();        
        $giorno = DayRepository::get_giorno();
        if($giorno!==true){
    ...
    public static function get_uscite_entrateByDate(string $giorno){
        $user = Authenticator::getUser();        
